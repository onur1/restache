...
const { runDecodeTest } = require('./_helpers/run')
...
const decodeTestCases = [
  ...
  },
  {
    ...
    expected: Element(undefined, { x: true }, [
      Element('a', {}, [Text('helloworldabcdef'), Variable('x'), Text('ghijkl')]),
    ]),
  },
  {
    desc: 'invalid tag id',
  ...
  {
    desc: 'section with predicate',
    input: [2, 'a', [], 'x > 1'],
    expected: Section(['a'], [], undefined, 'x > 1'),
  },
  {
    desc: 'invalid section identifier',
    input: [2, 'a(foo,)'],
    expectedErr: 'expected a valid section identifier, got "a(foo,)"',
  },
  {
    desc: 'section with index key & predicate',
    input: [2, 'a(foo)', [], 'x == 1'],
    expected: Section(['a'], [], 'foo', 'x == 1'),
  },
]
