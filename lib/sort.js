...
const getDependencies = e => {
  switch (e.value.type) {
    ...
    case ELEMENT:
      return (e.value.component ? [e.value.component] : []).concat(...e.forest.flatMap(getDependencies))
    case VARIABLE:
    case COMMENT:
    case TEXT:
      return []
...
const getDependencyGraph = trees => {
  ...
  trees.forEach(({ forest, value }) => {
    ...
    graph[component] = Vertex(component, deps.length > 1 ? uniq(deps) : deps)
  })
